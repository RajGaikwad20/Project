...
class BullyAlgorithm {
    ...
    private static int crashedProcess = -1; // Initialize with an invalid process ID
    ...
    private void determineCoordinator(int initiator) {
        ...
        for (int i = initiator + 1; i < numProcesses; i++) {
            if (activeProcesses[i] && i != crashedProcess) {
                if (i > initiator) {
    ...
    public static void main(String[] args) {
        ...
        System.out.print("Enter the process ID to crash (enter -1 for no crash): ");
        crashedProcess = scanner.nextInt();
        if (crashedProcess >= numProcesses || crashedProcess < -1) {
            System.out.println("Invalid process ID for crash.");
            crashedProcess = -1; // Reset to no crash if invalid input
        } else if (crashedProcess != -1) {
